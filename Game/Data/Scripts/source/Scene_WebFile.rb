...
class Scene_WebFile < Scene_MenuBase
  def main
  ...
  def create_savefile_windows
    @savefile_windows = Array.new(item_max) do |i|
      Window_WebSaveFile.new(savefile_height, i)
    end
  ...
  def update_savefile_selection
    return on_savefile_ok     if CInput.trigger?($vkey[:Check])
    return on_savefile_cancel if CInput.trigger?($vkey[:X])
    update_cursor
  end
  #--------------------------------------------------------------------------
  # ● 存档文件“确定”
